//! Production-hardened cognitive detection — Core Distance anomaly detection.
//!
...
impl CoreDistanceDetector {
    ...
    pub fn detect(&self, embedding: &[f32]) -> Result<DetectionResult, DetectionError> {
        ...
        let z_score =
            (core_dist - self.calibration_stats.mean) / self.calibration_stats.std.max(1e-10);
...
impl ContextAwareDetector {
    ...
    pub fn analyze(&self, text: &str, embedding: &[f32]) -> ContextualResult {
            ...
            ContextType::Mixed => {
                ...
                if mal_score.percentile_rank > acad_score.percentile_rank {
                    (
                        mal_score,
                        acad_score,
                        "Mixed context - using malicious baseline",
                    )
                } else {
                    (
                        acad_score,
                        mal_score,
                        "Mixed context - using academic baseline",
                    )
                }
            }
        };
        ...
        ContextualResult {
            ...
            reasoning: format!(
                "{}: {:.2} percentile",
                context, primary_score.percentile_rank
            ),
            primary_score,
            secondary_score,
        }
...
impl ContextClassifier {
    ...
    pub fn classify(&self, text: &str) -> ContextType {
        ...
        let acad_count = academic_markers
            .iter()
            .filter(|m| text_lower.contains(*m))
            .count();
        let op_count = operational_markers
            .iter()
            .filter(|m| text_lower.contains(*m))
            .count();
