...
const DEFAULT_TARGET_US: u64 = 1000;
...
async fn main() {
    let database_url = match std::env::var("DATABASE_URL") {
        ...
        _ => {
            eprintln!("DATABASE_URL is required for unlogged_insert_gate");
            std::process::exit(2);
        }
    };

    let mut samples = std::env::var("HALTCHAIN_BENCH_UNLOGGED_INSERT_SAMPLES")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(DEFAULT_SAMPLES);
    let mut target_p95_us = std::env::var("HALTCHAIN_BENCH_UNLOGGED_INSERT_P95_TARGET_US")
        .ok()
    ...
    let db = DbStore::connect(&database_url)
        .await
        .unwrap_or_else(|e| panic!("failed to connect db for unlogged insert gate: {e}"));

    let org_id = Uuid::new_v4();
    let agent_id = format!("unlogged-insert-gate-{}", Uuid::new_v4());
    ...
    for i in 0..samples {
        ...
        db.insert_telemetry_hot_sync(&rec)
            .await
            .unwrap_or_else(|e| panic!("insert_telemetry_hot_sync failed: {e}"));
    ...
    println!(
        "unlogged-insert gate: samples={samples} p95={p95}us target={target_p95_us}us (synchronous INSERT into telemetry_hot)"
    );
    if let Some(path) = json_out.as_deref() {
        let payload = json!({
            "gate": "unlogged_insert_gate",
            "measurement": "synchronous_insert",
            "samples": samples,
            "p95_us": p95,
    ...
    if p95 > target_p95_us {
        eprintln!("FAIL: unlogged insert p95 {p95}us exceeds target {target_p95_us}us");
        std::process::exit(1);
    }
    println!("PASS: unlogged insert p95 {p95}us <= target {target_p95_us}us");
}
